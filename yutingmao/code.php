...
// Create some colors
$bg = imagecolorallocate($im, 0, 128, 128); // 给画布加上背景颜色
...
$font = 'TNR.ttf';  // 设定字体文件的路径

$myimagecode = '';
for($i=0;$i<4;$i++){
   $str = $big_chars[rand(0,61)]; // 从合并后的数组中随机取一个字符
   $myimagecode = $myimagecode.$str;  // 将整个验证码字符串存入到变量myimagecode中
   $a = 50*$i;
   $b = 50*($i+1);
   imagettftext($im, 20, 0, mt_rand($a,$b), mt_rand(20,30), $black, $font, $str); // 将取出的字符写在画布上
}
$_SESSION['thisimagcode'] = $myimagecode;//将整个验证码字符串存入session变量中(为了便于在handle_login.php中进行验证码验证时被取到)
for($i=0;$i<200;$i++){
